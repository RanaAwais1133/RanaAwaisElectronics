...
// ✅ Action colors with more variants
const actionColors: Record<string, string> = {
  ...
  RESCHEDULE: 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/40 dark:text-yellow-300',
};
...
// ✅ Parse details string into key-value pairs
...
const AuditLogsPage: React.FC = () => {
  ...
  const totalPages = useMemo(() => Math.ceil(totalLogs / limit), [totalLogs]);
  ...
  const formatTime = (ts: string) => {
    if (!ts) return '';
    return new Date(ts).toLocaleTimeString(isUrdu ? 'ur-PK' : 'en-PK', {
      hour: '2-digit',
      minute: '2-digit',
      second: '2-digit',
    });
  ...
  const formatFullDate = (ts: string) => {
    if (!ts) return '';
    return new Date(ts).toLocaleDateString(isUrdu ? 'ur-PK' : 'en-PK', {
      year: 'numeric',
      month: 'long',
      day: 'numeric',
      hour: '2-digit',
  ...
  const LogCard = ({ log }: { log: any }) => (
    <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200/60 dark:border-gray-700/60 p-4 space-y-3 shadow-sm">
      ...
      <div className="flex items-center justify-between text-xs">
        <span className="font-mono text-gray-500">{truncateText(log.entity_id, 12) || '—'}</span>
        ...
        <div className="flex items-center gap-2">
          {log.user_name ? (
            <>
              ...
              <span className="font-semibold text-gray-800 dark:text-gray-100 text-xs truncate max-w-[80px]">
                {log.user_name}
              </span>
            </>
          ) : (
    ...
    <div className="max-w-7xl mx-auto px-3 sm:px-4 pb-10 space-y-4">
      ...
      <div className="flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            ...
            className={`w-full ${isUrdu ? 'pr-12 pl-4' : 'pl-12 pr-4'} py-2.5 border border-gray-300 dark:border-gray-600 rounded-2xl bg-white dark:bg-gray-800 text-sm focus:ring-2 focus:ring-blue-400 focus:border-transparent outline-none transition-colors`}
          />
        </div>
        ...
        <select
          ...
          className="border border-gray-300 dark:border-gray-600 rounded-2xl px-4 py-2.5 bg-white dark:bg-gray-800 text-sm focus:ring-2 focus:ring-blue-400 focus:border-transparent outline-none transition-colors"
        >
        ...
        {(search || filterAction || filterEntity) && (
          <button
            ...
            className="px-4 py-2.5 text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-2xl transition-all"
          >
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-white dark:bg-gray-800 rounded-3xl border border-dashed border-gray-300 dark:border-gray-600">
          ...
          <button onClick={() => fetchLogs(page)} className="text-blue-600 font-semibold underline hover:no-underline">
            {t('retry')}
          </button>
        </div>
      ) : filtered.length === 0 ? (
            ...
            <div className="overflow-x-auto max-h-[70vh] overflow-y-auto">
              <table className="w-full min-w-[800px] text-sm">
                <thead className="bg-gradient-to-r from-gray-50 to-gray-100 dark:bg-gray-700 sticky top-0 z-10">
                  <tr>
                    <th className="px-5 py-4 text-start text-xs font-bold text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                      {t('action')}
                    </th>
                    <th className="px-5 py-4 text-start text-xs font-bold text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                      {t('entity')}
                    ...
                    <th className="px-5 py-4 text-center text-xs font-bold text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-700/50">
                  {filtered.map((log: any, idx: number) => (
                    <tr
                      key={idx}
                      className={`hover:bg-blue-50/30 dark:hover:bg-blue-900/10 transition-all duration-200 ${
                      ...
                      <td className="px-5 py-4">
                        {log.user_name ? (
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-semibold text-gray-800 dark:text-gray-100 truncate max-w-[100px]">
                              {log.user_name}
                            </span>
                          </div>
                        ) : (
      ...
      {detailModal && (
        ...
        >
          <div
            className="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl w-full max-w-lg max-h-[85vh] overflow-y-auto mx-2"
            onClick={e => e.stopPropagation()}
          >
            <div className="sticky top-0 bg-white dark:bg-gray-800 z-10 flex justify-between items-center px-5 py-4 border-b border-gray-200 dark:border-gray-700 rounded-t-3xl">
              <h3 className="text-lg font-bold text-gray-800 dark:text-white">
                📋 {isUrdu ? 'مکمل تفصیل' : 'Full Details'}
              </h3>
              <button
                onClick={() => setDetailModal(null)}
                className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 text-xl transition-colors"
              >
                ✕
              </button>
            </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm font-semibold text-gray-800 dark:text-gray-100">
                    {entityIcons[detailModal.entity] || '📄'} {detailModal.entity}
                  </p>
                </div>
                <div>
                  <p className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">ID</p>
                  <p className="text-sm font-mono text-gray-800 dark:text-gray-100 break-all">
                    {detailModal.entity_id || '—'}
                  </p>
                </div>
                <div>
                  <p className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">
                  ...
                  <p className="text-sm font-semibold text-gray-800 dark:text-gray-100">
                    {detailModal.user_name || detailModal.user_id?.slice(-8) || '—'}
                  </p>
                </div>
                <div className="col-span-2">
                  <p className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">
                    {isUrdu ? 'تاریخ و وقت' : 'Timestamp'}
                  </p>
                  <p className="text-sm text-gray-800 dark:text-gray-100">
                    {formatFullDate(detailModal.timestamp)}
                  </p>
                </div>
              </div>
                ...
                <div className="bg-gray-50 dark:bg-gray-900/50 rounded-xl p-4">
                  {detailModal.details ? (
                    ...
                    <p className="text-sm text-gray-400 italic">
                      {isUrdu ? 'کوئی تفصیل نہیں' : 'No details available'}
                    </p>
                  )}
                </div>
              </div>
            ...
            <div className="sticky bottom-0 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 px-5 py-3 flex justify-end rounded-b-3xl">
              <button
                onClick={() => setDetailModal(null)}
                className="px-5 py-2 bg-gray-100 dark:bg-gray-700 rounded-xl text-sm font-semibold text-gray-700 dark:text-gray-200 hover:bg-gray-200 dark:hover:bg-gray-600 transition-all"
              >
