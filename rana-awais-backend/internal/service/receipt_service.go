...
func (s *ReceiptService) GenerateAndPrintReceipt(ctx context.Context, paymentID primitive.ObjectID) error {
	...
	// Build detailed bilingual receipt for thermal printer
	headerEn := "RANA AWAIS ELECTRONICS\nPayment Receipt"
	headerUr := "رانا اویس الیکٹرانکس\nادائیگی رسید"
	...
	bodyEn += "\nThank you for choosing Rana Awais Electronics!"
	bodyEn += "\nSoftware by: Huzaifa Qasim (0313-6487199)"

	bodyUr := fmt.Sprintf("گاہک: %s\n", cust.NameUrdu)
	if cust.FatherName != "" {
		bodyUr += fmt.Sprintf("ولدیت: %s\n", cust.FatherName)
	}
	...
	bodyUr += "\nرانا اویس الیکٹرانکس پر اعتماد کے لیے شکریہ!"
	bodyUr += "\nسافٹ ویئر بذریعہ: حذیفہ قاسم (0313-6487199)"
