...
// ═══════════════════════════════════════════════════════════════
// PRODUCTS GROUPED BY NAME - returns unique product names with inventory count
// ═══════════════════════════════════════════════════════════════

func (h *DashboardHandler) ProductsGrouped(w http.ResponseWriter, r *http.Request) {
	db := getDB()
	if db == nil {
		respondJSON(w, http.StatusOK, []interface{}{})
		return
	...
	// Get all unique product names from products collection
	...
	type productGroup struct {
		...
		ProductNameUrdu string  `json:"productNameUrdu"`
		Category        string  `json:"category"`
		TotalStock      int     `json:"totalStock"`
		InStockItems    int     `json:"inStockItems"`
		SoldItems       int     `json:"soldItems"`
		TotalValue      float64 `json:"totalValue"`
	}

	productMap := make(map[string]*productGroup)
	for prodCursor.Next(ctx()) {
		var prod domain.Product
		...
		pg := &productGroup{
			...
			InStockItems:    0,
			SoldItems:       0,
			TotalValue:      float64(prod.StockCount) * prod.PurchasePrice,
		}
		productMap[prod.ID] = pg
	}

	// Count inventory items per product by status
	inventoryCursor, err := db.Collection("inventory_items").Find(ctx(), bson.M{})
	if err == nil {
		defer inventoryCursor.Close(ctx())
		for inventoryCursor.Next(ctx()) {
			var item domain.InventoryItem
			if inventoryCursor.Decode(&item) != nil {
				continue
			}
			if pg, ok := productMap[item.ProductID]; ok {
				pg.TotalStock++
				...
				pg.TotalValue += item.PurchasePrice
			}
		}
	}
	...
	for _, pg := range productMap {
		result = append(result, pg)
	}
	// Sort by stock count descending
	for i := 0; i < len(result); i++ {
		for j := i + 1; j < len(result); j++ {
			if result[j].TotalStock > result[i].TotalStock {
