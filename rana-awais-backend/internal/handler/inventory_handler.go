...
func (h *InventoryHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	if input.ProductID != "" { existing.ProductID = input.ProductID }
	if input.SerialNumber != "" { existing.SerialNumber = input.SerialNumber }
	if input.Color != "" { existing.Color = input.Color }
	if input.Model != "" { existing.Model = input.Model }
	if input.EngineNo != "" { existing.EngineNo = input.EngineNo }
	if input.ChassisNo != "" { existing.ChassisNo = input.ChassisNo }
	...
	audit.Log(r.Context(), "UPDATE", "inventory", id, "", getUserID(r))
...
func (h *InventoryHandler) AddStock(w http.ResponseWriter, r *http.Request) {
	...
	// ✅ Update product stock_count and in_stock flag
	prod, err := h.prodSvc.GetByID(r.Context(), payload.ProductID)
	if err == nil && prod != nil {
		prod.StockCount += payload.Quantity
